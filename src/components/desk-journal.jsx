    ...
    <div className="w-full animate-fade-in-up">
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 sm:px-5 py-2 sm:py-2.5 rounded-xl text-[9px] sm:text-[11px] font-black uppercase tracking-wider whitespace-nowrap transition-all duration-300 cursor-pointer shrink-0 ${
                  isActive
                    ? 'bg-emerald-950 text-emerald-400 border-none shadow-md scale-[1.02]'
        ...
        {menuTabs.includes(activeTab) && (
          <div className="relative shrink-0 w-full xl:w-auto">
            ...
            <input
              ...
              className="w-full xl:w-72 pl-11 pr-5 py-3 text-xs font-bold border-2 border-transparent bg-white/70 backdrop-blur-md rounded-2xl shadow-sm focus:outline-none focus:border-emerald-700/30 focus:bg-white transition-all placeholder-stone-400 text-stone-700"
            />
          </div>
        )}
      </div>
      ...
      {menuTabs.includes(activeTab) && (
        <div className="flex items-end justify-between mb-8 pb-4 border-b border-stone-200/60">
          <div>
            <span className="text-[10px] tracking-[0.25em] font-bold uppercase text-emerald-800 block mb-2">Our Menu</span>
      ...
      {menuTabs.includes(activeTab) ? (
        displayedItems.length > 0 ? (
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-5">
            {displayedItems.map((item, idx) => {
              ...
              return (
                ...
                >
                  <div className="flex flex-col bg-white/80 backdrop-blur-xl rounded-[16px] sm:rounded-[28px] overflow-hidden shadow-[0_8px_24px_rgba(4,120,87,0.04)] border border-white/60 sm:border-2 hover:shadow-[0_20px_40px_rgba(4,120,87,0.12)] hover:-translate-y-1 sm:hover:-translate-y-2 transition-all duration-500 h-full relative min-w-0">
                    
                    {/* Glowing background accent behind image */}
                    <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-emerald-100/50 to-transparent z-0 pointer-events-none" />
                    ...
                    <div className="relative h-28 xs:h-36 sm:h-56 md:h-64 overflow-hidden bg-stone-100 shrink-0 z-10 m-1.5 sm:m-2 rounded-[12px] sm:rounded-[20px]">
                      ...
                      {isBestSeller && (
                        <div className="absolute top-2 left-2 sm:top-3 sm:left-3 flex items-center gap-1 sm:gap-1.5 bg-white/95 backdrop-blur-md text-emerald-800 text-[7px] sm:text-[9.5px] font-black uppercase tracking-wider px-2 py-1 sm:px-3.5 sm:py-2 rounded-lg sm:rounded-xl shadow-lg border border-white">
                          <Sparkles className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 text-emerald-600" />
                          Top Pick
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex flex-col flex-1 p-3 sm:p-5 md:p-6 gap-1.5 sm:gap-3 z-10">
                      <div>
                        {/* Name */}
                        <p className="font-serif font-black text-stone-900 text-[13px] sm:text-xl md:text-2xl leading-snug group-hover:text-emerald-800 transition-colors line-clamp-1">
                          {item.name}
                        </p>
                        {/* Tags - 1 tag on mobile, 2 on sm+ */}
                        <div className="flex items-center gap-1 sm:gap-1.5 mt-1.5 sm:mt-2.5 overflow-hidden">
                          {item.tags.slice(0, 1).map(tag => (
                            <span key={tag} className="text-[6.5px] sm:text-[10.5px] uppercase tracking-wider sm:tracking-widest font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded sm:rounded-md whitespace-nowrap shrink-0">
                              {tag}
                            </span>
                          ))}
                          {item.tags.length > 1 && (
                            <span className="hidden sm:inline-block text-[10.5px] uppercase tracking-widest font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-md whitespace-nowrap shrink-0">
                              {item.tags[1]}
                            </span>
                          )}
                        </div>
                      ...
                      <div className="flex items-end justify-between pt-2 sm:pt-5 mt-auto">
                        <div>
                          ...
                          <span className="font-serif font-black text-stone-900 text-base sm:text-2xl md:text-3xl leading-none block">
                            <span className="text-emerald-800 mr-0.5">₱</span>{item.price}
                          </span>
                        </div>
                        <div className="w-8 h-8 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-emerald-950 text-emerald-400 flex items-center justify-center transform group-hover:scale-110 group-hover:bg-emerald-800 group-hover:shadow-[0_8px_20px_rgba(4,120,87,0.3)] transition-all duration-300 shadow-md">
          ...
          <div className="flex flex-col items-center py-24 gap-3">
            ...
            <button onClick={() => setSearchQuery('')} className="text-xs font-black text-emerald-700 cursor-pointer hover:underline underline-offset-2">
              Clear search
            </button>
          </div>
        )
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {additionalServices.map((service, idx) => (
              <div key={idx} className="flex flex-col bg-white border border-stone-100 rounded-2xl p-5 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300">
                <div className="w-9 h-9 bg-emerald-50 rounded-xl flex items-center justify-center shrink-0 mb-3">
                ...
                <span className="inline-block self-start text-[8px] bg-emerald-50 text-emerald-700 font-black uppercase tracking-wide px-2.5 py-0.5 rounded-full border border-emerald-100 mt-3">
                  Free
                </span>
              </div>
            ))}
